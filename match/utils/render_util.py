...
import pyrender
import trimesh
from match.utils import image_util
import torch
...
def render_mesh(
    *,
    ...
    camera_distortions: _FloatArray | None = None,  # unused
    multisample_antialiasing: int = 1,  # unused
    background_color: _FloatArray | None = None,
    enable_cull_face: bool = False,
    vertex_colors: _FloatArray | None = None,
    face_colors: _FloatArray | None = None,
  ...
  for i in range(batch_size):
    ...
    color = image_util.increase_contrast(color, contrast)
...
def _build_intrinsics_and_extrinsics(sample: dict) -> tuple[np.ndarray, np.ndarray]:
    c2w = _to_numpy(sample["C2W"]).astype(np.float32)  # (V, 4, 4)
    fxfycxcy = _to_numpy(sample["fxfycxcy"]).astype(np.float32)  # (V, 4), normalized
    _, h, w = _to_numpy(sample["image"])[0].shape

    extrinsics = np.linalg.inv(c2w)[:, :3, :4].astype(np.float32)

    intrinsics = np.zeros((fxfycxcy.shape[0], 2, 3), dtype=np.float32)
    intrinsics[:, 0, 0] = fxfycxcy[:, 0] * w
    intrinsics[:, 1, 1] = fxfycxcy[:, 1] * h
    intrinsics[:, 0, 2] = fxfycxcy[:, 2] * w
    intrinsics[:, 1, 2] = fxfycxcy[:, 3] * h
    return extrinsics, intrinsics


def render_mesh_from_sample(sample: dict, faces: np.ndarray, contrast: float) -> np.ndarray:
    coarse_verts = _to_numpy(sample["coarse_verts"]).astype(np.float32)
    if coarse_verts.shape[0] == 0:
        raise ValueError("Sample has empty `coarse_verts`. Check coarse mesh directory.")

    extrinsics, intrinsics = _build_intrinsics_and_extrinsics(sample)
    num_views = extrinsics.shape[0]
    _, h, w = _to_numpy(sample["image"])[0].shape

    rendered = render_mesh(
        vertices=np.repeat(coarse_verts[None, ...], repeats=num_views, axis=0),
        faces=faces,
        camera_extrinsics=extrinsics,
        camera_intrinsics=intrinsics,
        image_size=(h, w),
        camera_distortions=np.zeros((num_views, 5), dtype=np.float32),
        background_color=np.array([1.0, 1.0, 1.0], dtype=np.float32),
        enable_cull_face=False,
        contrast=contrast,
    )
    rendered = np.clip(np.round(rendered * 255.0), 0, 255).astype(np.uint8)  # (V, H, W, 3)  [0, 255]
    return rendered
