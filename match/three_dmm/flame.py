...
import trimesh
from match.utils import mesh_util


self_path = 'assets'

# FLAME_MODEL_PATH = "asset/flame/generic_model.pkl"
FLAME_MODEL_PATH = f"{self_path}/flame/FLAME_2023.pkl"
FLAME_MESH_PATH = f"{self_path}/flame/head_template_mesh.obj"
FLAME_PARTS_PATH = f"{self_path}/flame/FLAME_masks.pkl"
FLAME_LMK_PATH = f"{self_path}/flame/landmark_embedding_with_eyes.npy"
FLAME_TEX_PATH = f"{self_path}/flame/FLAME_texture.npz"
FLAME_PAINTED_TEX_PATH = f"{self_path}/flame/tex_mean_painted.png"
FLAME_UVMASK_PATH = f"{self_path}/flame/uv_masks.npz"
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        # Just to avoid using pytorch3d
        template_info = mesh_util.load_obj(str(flame_template_mesh_path))
        faces_vert_idcs = template_info['vi']
        faces_uv_idcs = template_info['vti']
        uv_coords = template_info['vt']
        ...
        self.register_buffer("faces", torch.from_numpy(faces_vert_idcs), persistent=False)
        self.register_buffer("faces_uv", torch.from_numpy(faces_uv_idcs), persistent=False)
        self.register_buffer("uv_coords", torch.from_numpy(uv_coords), persistent=False)
