...
# from google3.research.vale.tools.image_scrape import remote_file_wrapper
...
# # GOOGLE
# class AccelerateRemoteDirWrapper(remote_file_wrapper.RemoteDirWrapper):
...
#         self.Flush()
#         self._file_interface.DeleteRecursively(self._local_tmp_filename)
#     self._accelerator.wait_for_everyone()

#   def Flush(self):
#     """Writes results back to remote file."""
#     if self._should_writeback:
#       self._file_interface.RecursivelyCopyDir(
#           self._local_tmp_filename, self._remote_filename, True
#       )

#   def GetFilename(self):
#     self._accelerator.wait_for_everyone()
#     # Copy only if we intend to use the file.
#     if not self._copied_locally and self._file_interface.Exists(
...
#         if self._file_interface.Exists(self._local_tmp_filename):
#           self._file_interface.DeleteRecursively(self._local_tmp_filename)
#         self._file_interface.MakeDirs(os.path.dirname(self._local_tmp_filename))
#         self._file_interface.RecursivelyCopyDir(
#             self._remote_filename, self._local_tmp_filename, True
#         )
...
def open_file(file_path: Union[Path, str], *args, **kwargs):
  """Opens a file."""
...
def get_sub_directories(file_path: Path):
...
def list_dir(file_path: Union[Path, str]) -> list:
  """Get list of paths of all files in a directory."""
  return os.listdir(str(file_path))


def makedirs(file_path: Union[Path, str], exist_ok: bool = False) -> None:
  """Make directories."""
  os.makedirs(str(file_path), exist_ok=exist_ok)


def is_directory(file_path: Union[Path,str]):
...
def copy(
    file_path: Union[Path, str], target_path: Union[Path, str], overwrite: bool = False
) -> None:
...
def exists(file_path: Union[Path, str]) -> bool:
...
def glob(file_path: Union[Path,  str], pattern: str) -> list:
  """Glob files.
...
def save_npz(output_path: Union[Path, str], data) -> None:
