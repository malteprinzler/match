...
class Basic3DBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, norm='bn'):
        ...
        self.block = nn.Sequential(
            nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, stride=1, padding='same', bias=False),
...
class Res3DBlock(nn.Module):
    def __init__(self, in_planes, out_planes, norm='bn'):
        ...
        self.res_branch = nn.Sequential(
            nn.Conv3d(in_planes, out_planes, kernel_size=3, stride=1, padding='same'),
            nn.BatchNorm3d(out_planes) if norm == 'bn' else nn.InstanceNorm3d(out_planes),
            nn.ReLU(True),
            nn.Conv3d(out_planes, out_planes, kernel_size=3, stride=1, padding='same'),
            nn.BatchNorm3d(out_planes) if norm == 'bn' else nn.InstanceNorm3d(out_planes)
        )

        self.skip_con = nn.Sequential(
                nn.Conv3d(in_planes, out_planes, kernel_size=1, stride=1, padding='same'),
                nn.BatchNorm3d(out_planes) if norm == 'bn' else nn.InstanceNorm3d(out_planes)
            )
...
class EncoderDecorder(nn.Module):
    def __init__(self, level=5, norm='bn'):
        ...
        if self.level >= 1:
            ...
            self.decoder_upsample1 = Upsample3DBlock(32, 32, 2, 2, norm=norm)

        if self.level >= 2:
            self.encoder_pool2 = Pool3DBlock(2)
            self.decoder_upsample2 = Upsample3DBlock(64, 64, 2, 2, norm=norm)
        ...
        self.decoder_res2 = Res3DBlock(128, 64, norm=norm)
        self.decoder_res1 = Res3DBlock(64, 32, norm=norm)
...
class V2VModel(nn.Module):
    def __init__(self, input_channels, output_channels, encdec_level=4, decoder_scale_factor=1, norm='bn'):
        super().__init__()
