...
class VolumetricFeatureSampler(nn.Module):
    ...
    def __init__(self, padding_mode='border', feature_fusion='', feature_dim=0, interpolation_mode='nearest'):
    ...
    def baseline_mean_var(self, feature_maps, camera_intrinsics, camera_extrinsics, camera_distortions, grids):
        ...
        for opid, op in enumerate(self.fuse_ops):  # operation
            ...
            for view_idx in range(num_views):
                ...
                feat2d_uv = F.grid_sample(feature_maps[:, view_idx], grid2d_uv, padding_mode=self.padding_mode, align_corners=True, mode=self.interpolation_mode)  # (B, F, G, GH*GW*GD)
    ...
    def mean_var(self, feature_maps, camera_intrinsics, camera_extrinsics, camera_distortions, grids):
        ...
        feat2d_uv = F.grid_sample(feature_maps.view(-1, fd, height, width), grid2d_uv, padding_mode=self.padding_mode, align_corners=True, mode=self.interpolation_mode)  # (B*V, F, G, GH*GW*GD)
...
###########
    def debug_feature_sampling(self, feature_maps, camera_intrinsics, camera_extrinsics, camera_distortions, points):
        ...
        feat2d_uv = F.grid_sample(feature_maps.view(-1, fd, height, width), grid2d_uv, padding_mode=self.padding_mode, align_corners=True, mode=self.interpolation_mode)  # (B*V, F, G, 1)
    ...
    def visualize_first_sample_scene(
        self,
        camera_extrinsics,
        grids,
        outpath,
        ...
        """Visualize first batch sample as a 3D Plotly scene.

        Args:
        - camera_extrinsics: tensor in (B, V, 3, 4), world-to-camera extrinsics
        - grids: tensor in (B, G, 3, GH, GW, GD), world-space grid coordinates
        - outpath: output file path (without extension), saves "{outpath}_3d.html"
        - camera_ids: optional iterable with V camera IDs, defaults to stringified indices
        - sample_name: name suffix shown in plot labels
        - axis_length: length of camera local axes
        - grid_vis_stride: controls random downsampling amount for grid display
        """
        try:
            import numpy as np
            ...
            raise ImportError(
                "Plotly and numpy are required for scene visualization. "
                "Please install them to use visualize_first_sample_scene()."
            ) from exc

        if camera_extrinsics.shape[0] < 1 or grids.shape[0] < 1:
            raise ValueError("Expected non-empty batch for camera_extrinsics and grids.")

        # First sample in batch.
        extr = camera_extrinsics[0].detach().cpu()  # (V, 3, 4)
        grid = grids[0].detach().cpu()              # (G, 3, GH, GW, GD)
        num_views = extr.shape[0]

        if camera_ids is None:
            camera_ids = [str(i) for i in range(num_views)]
        else:
            camera_ids = [str(cid) for cid in camera_ids]
            if len(camera_ids) != num_views:
                raise ValueError(
                    f"camera_ids length ({len(camera_ids)}) does not match number of views ({num_views})."
                )

        # Flatten world-space grid coordinates to points for scatter rendering.
        # grid: (G, 3, GH, GW, GD) -> (N, 3), then randomly subsample for visualization.
        grid_points = grid.permute(0, 2, 3, 4, 1).reshape(-1, 3).numpy()
        sample_factor = max(1, int(grid_vis_stride))
        num_points = grid_points.shape[0]
        num_samples = max(1, num_points // sample_factor)
        if num_samples < num_points:
            sampled_idx = np.random.choice(num_points, size=num_samples, replace=False)
            grid_points = grid_points[sampled_idx]
        ...
        fig_3d.add_trace(
            go.Scatter3d(
                ...
                name=f"Grid_{sample_name}",
            )
        )

        # Convert world-to-camera [R|t] to camera-to-world:
        # x_cam = R * x_world + t
        # C (camera center in world) = -R^T * t, R_c2w = R^T
        for i in range(num_views):
            ...
            x, y, z = origin
            el = np.rad2deg(np.arctan2(-y, (x ** 2 + z ** 2) ** 0.5))
            ...
            for dir_vec, color, axis_name in zip(
                [x_dir, y_dir, z_dir],
                ["red", "green", "blue"],
                ["X", "Y", "Z"],
            ):
                ...
                fig_3d.add_trace(
                    go.Cone(
                        ...
                        sizemode="absolute",
                        sizeref=axis_length / 5.0,
                        name=f"Cam{camera_ids[i]}_{sample_name}_{axis_name}",
                    )
                )
        ...
        fig_3d.update_layout(
            ...
            title="Camera Extrinsics and Grid",
        )
