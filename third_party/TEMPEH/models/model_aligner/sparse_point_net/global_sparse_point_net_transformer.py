...
class Model(BaseModel):
    ...
    def forward(self, feature_maps, camera_intrinsics, camera_extrinsics, camera_distortions, random_grid=True):
        ...
        if False: # skipping grid refinement
            ...
            warped_grid = global_grid
            transformed_grid_origin = self.global_origin[0].repeat(bs, 1).to(device)
            grid_scales = torch.ones(bs, 3).to(device)
