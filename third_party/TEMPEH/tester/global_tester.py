...
class Tester(BaseTester):
    ...
    def register_dataset(self):
        ...
        world_size = int(os.environ.get('CONDOR_WorldSize', '1'))
        job_idx = int(os.environ.get('CONDOR_Process', '0'))
        print(f'World Size: {world_size}, Process Idx: {job_idx}')
        self.dataset = AvaMultiCaptureDataset(
            root_path=ava_root, 
    ...
    def run(self):
        ...
        self.set_eval()
        with torch.no_grad():
            for data in tqdm.tqdm(self.dataloader, 'Testing'):
                self.feed_data(data)
                # time1 = time.time_ns()                
                self.forward()
                # time2 = time.time_ns()                
    ...
    def export_data(self):
        ...
        # save prediction: 
        global_points = self.global_points.detach().cpu().numpy()
        global_points[..., -2:] = -1 * global_points[..., -2:]
        global_points /= 1000.0
        B=len(global_points)
        ...
        global_points = data_utils.apply_inv_rotation_scale_center_to_points(global_points, rotation=ds_rotation, scale=ds_scale_factor, center=ds_center)
        reconstructed_vertices = global_points
        batch_size = len(global_points)
        faces = self.mesh_sampler.get_mesh(-1).f
        to_meter_scale_factor = 1.
