...
class PointToSurfaceLoss(nn.Module):
    ...
    def forward(self, points, mesh_vertices, mesh_faces, return_distances=False):
        ...
        # # visualize error map as 3d scatter plot
        # from gtempeh_utils import vis_util
        # # fig = vis_util.vis_3d_meshes(mesh_vertices=[points[0].detach().cpu().numpy(), mesh_vertices[0].detach().cpu().numpy()], 
        # #                              mesh_faces = [mesh_faces.detach().cpu().numpy()]*2,
        # #                              mesh_names=['target', 'prediction'])
        # # fig.write_html('demos/p2s_meshes.html')
        # fig = vis_util.vis_3d_point_cloud_scores(points=points[0, ::10].detach().cpu().numpy(), scores=robust_distances[0, ::10].detach().cpu().numpy(), mesh_faces=mesh_faces.cpu().numpy(), mesh_vertices=mesh_vertices[0].detach().cpu().numpy())
        # fig.write_html('demos/p2s_metric.html')
        # import pudb
        # pudb.set_trace()
        # print()
