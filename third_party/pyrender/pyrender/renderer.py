...
class Renderer(object):
    ...
    def __init__(self, viewport_width, viewport_height, point_size=1.0, antialias=True):
        self.dpscale = 1
    ...
    def read_color_buf(self):
        ...
        if sys.platform == 'darwin':
            color_im = self._resize_image(color_im, self.antialias)
    ...
    def read_depth_buf(self):
        ...
        # Resize for macos if needed
        if sys.platform == 'darwin':
            depth_im = self._resize_image(depth_im, self.antialias)
    ...
    def _forward_pass(self, scene, flags, seg_node_map=None):
        ...
        if (not bool(flags & RenderFlags.SEG)) and self.antialias:
