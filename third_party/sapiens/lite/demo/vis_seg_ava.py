...
def img_save_and_viz(
    ...
    output_path_vis = output_path.split('.')[0] + '_vis.jpg'
    ...
    Image.fromarray(pred_sem_seg.astype(np.uint8)).save(output_path)
    ...
    vis_image = np.concatenate([image_rgb, vis_image], axis=1)
    vis_height = 256
    vis_width = int(vis_height * vis_image.shape[1]/vis_image.shape[0])
    Image.fromarray(vis_image).resize((vis_width, vis_height)).save(output_path_vis)
...
def main():
    ...
    for batch_idx, (batch_zip_image_path, batch_orig_imgs, batch_imgs) in tqdm(
        ...
        for p in batch_zip_image_path[:valid_images_len]:
            ...
            out_paths.append(os.path.join(args.output_root, capture_str, cam_frame_str+'.png'))
