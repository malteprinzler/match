    ...
    def freezer(self, layers):
        for name in layers:
            if hasattr(self, name):
                ...
                logger.info(f"[RENSET] disabling grad for {name.upper()}")
...
def load_ResNet18Model(pretrained=True):
    model = ResNet(Bottleneck, [2, 2, 2, 2])
    logger.info("Loading ResNet18")
    if pretrained:
        copy_parameter_from_resnet(model, torchvision.models.resnet18(weights="ResNet18_Weights.DEFAULT").state_dict())
    return model


def load_ResNet34Model(pretrained=True):
    model = ResNet(Bottleneck, [3, 4, 6, 3])
    logger.info("Loading ResNet34")
    if pretrained:
        copy_parameter_from_resnet(model, torchvision.models.resnet34(weights="ResNet34_Weights.DEFAULT").state_dict())
    return model
...
def load_ResNet50Model(pretrained=True):
    ...
    if pretrained:
        copy_parameter_from_resnet(model, torchvision.models.resnet50(weights="ResNet50_Weights.DEFAULT").state_dict())
    return model


def load_ResNet101Model(pretrained=True):
    model = ResNet(Bottleneck, [3, 4, 23, 3])
    logger.info("Loading ResNet101")
    if pretrained:
