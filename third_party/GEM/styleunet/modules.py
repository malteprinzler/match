...
from styleunet.stylegan2_ops import upfirdn2d
...
class InverseHaarTransform(nn.Module):
    ...
    def forward(self, input, contiguous=False):
        ll, lh, hl, hh = input.chunk(4, 1)
        if contiguous:
            ll = upfirdn2d(ll.contiguous(), self.ll, up=2, pad=(1, 0))
            lh = upfirdn2d(lh.contiguous(), self.lh, up=2, pad=(1, 0))
            hl = upfirdn2d(hl.contiguous(), self.hl, up=2, pad=(1, 0))
            hh = upfirdn2d(hh.contiguous(), self.hh, up=2, pad=(1, 0))
        else:
...
class ConvBlock_latent(nn.Module):
    def __init__(self, in_channel, out_channel, style_dim, blur_kernel=[1, 3, 3, 1], downsample=True):
        super().__init__()
        self.conv1 = ModulatedConv2d(in_channel, in_channel, 3, style_dim, upsample=False, downsample=False)
        self.conv2 = ModulatedConv2d(in_channel, out_channel, 3, style_dim, upsample=False, downsample=downsample)
