...
def strip_lowerdiag(L):
    uncertainty = th.zeros((L.shape[0], 6), dtype=th.float, device="cuda")
...
def build_rotation(r):
    norm = th.sqrt(r[:, 0] * r[:, 0] + r[:, 1] * r[:, 1] + r[:, 2] * r[:, 2] + r[:, 3] * r[:, 3])

    q = r / norm[:, None]

    R = th.zeros((q.size(0), 3, 3), device="cuda")
...
def build_scaling_rotation(s, r):
    L = th.zeros((s.shape[0], 3, 3), dtype=th.float, device="cuda")
    R = build_rotation(r)
