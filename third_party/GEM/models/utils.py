...
class LinearELR(nn.Module):
    ...
    def __init__(self, inch, outch, lrmult=1.0, norm: Optional[str] = None, act=None):
        ...
        self.bias = nn.Parameter(torch.full([outch], 0.0))
...
def blockinit(k, stride):
    dim = k.ndim - 2
    return (
        k.view(k.size(0), k.size(1), *(x for i in range(dim) for x in (k.size(i + 2), 1)))
...
class ConvTranspose1dELR(nn.Module):
    def __init__(
        self, inch, outch, kernel_size, stride, padding, wsize=0, affinelrmult=1.0, norm=None, ub=None, act=None
    ):
        ...
        initgain = stride**0.5 if norm == "demod" else 1.0 / math.sqrt(fan_in)

        self.weightgain = actgain * initgain

        self.weight = nn.Parameter(blockinit(torch.randn(inch, outch, kernel_size // self.stride), self.stride))
    ...
    def forward(self, x, w: Optional[torch.Tensor] = None):
        ...
        if self.affine is not None and w is not None:
            ...
            weight = self.weight * (affine * 0.1 + 1.0)
        else:
            weight = self.weight
        ...
        out = F.conv_transpose1d(x, weight, None, stride=self.stride, padding=self.padding, dilation=1, groups=groups)
...
class ConvTranspose2dELR(nn.Module):
    def __init__(
        ...
        initgain = stride if norm == "demod" else 1.0 / math.sqrt(fan_in)

        self.weightgain = actgain * initgain

        self.weight = nn.Parameter(
            blockinit(torch.randn(inch, outch, kernel_size // self.stride, kernel_size // self.stride), self.stride)
        )
    ...
    def forward(self, x, w: Optional[torch.Tensor] = None):
        ...
        out = F.conv_transpose2d(x, weight, None, stride=self.stride, padding=self.padding, dilation=1, groups=groups)
...
class ConvTranspose3dELR(nn.Module):
    def __init__(
        ...
        initgain = stride**1.5 if norm == "demod" else 1.0 / math.sqrt(fan_in)
        ...
        self.weight = nn.Parameter(
            blockinit(
                torch.randn(
                    inch, outch, kernel_size // self.stride, kernel_size // self.stride, kernel_size // self.stride
                ),
                self.stride,
            )
        )
    ...
    def forward(self, x, w: Optional[torch.Tensor] = None):
        ...
        out = F.conv_transpose3d(x, weight, None, stride=self.stride, padding=self.padding, dilation=1, groups=groups)
...
class Conv2dELR(nn.Module):
    def __init__(
        ...
        initgain = 1.0 if norm == "demod" else 1.0 / math.sqrt(fan_in)

        self.weightgain = actgain * initgain

        self.weight = nn.Parameter(torch.randn(outch, inch, kernel_size, kernel_size))
    ...
    def forward(self, x, w: Optional[torch.Tensor] = None):
        ...
        out = F.conv2d(x, weight, None, stride=self.stride, padding=self.padding, dilation=1, groups=groups)
...
def matrix_to_axisangle(r):
    ...
    vec = (
        0.5
        * torch.stack([r[..., 2, 1] - r[..., 1, 2], r[..., 0, 2] - r[..., 2, 0], r[..., 1, 0] - r[..., 0, 1]], dim=-1)
        / torch.sin(th)
    )
    return th, vec
