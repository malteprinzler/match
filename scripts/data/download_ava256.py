        ...
        try:
            with ZipFile(path, 'r') as zf:
                if len(zf.namelist()) == 0:
                    logging.warning(f"Invalid file {path}. File is empty.")
                    return False

        except (FileNotFoundError, BadZipFile) as e:
            logging.warning(f"Invalid file {path}. {e}")
            return False
...
def download_link(from_url_and_to_path: Tuple[int, int, str, str]) -> None:
    ...
    if to_path.exists():
        ...
        logging.info("%s already exists, validating...", to_path)

        is_valid = validate_file(str(to_path))
        if is_valid:
            logging.info("%s is valid, skipping", to_path)
            return
        else:
            logging.info("%s is invalid", to_path)
...
def main():
    ...
    parser.add_argument("-n", type=int, default=None, help="Number of captures from captures-file download")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="If specified, only download these camera IDs for multi-camera assets (image, segmentation_parts, foreground_masks). Default: all cameras.",
    )
    ...
    for capture in captures:
        ...
        for assets, asset_paths in args.assets.items():
            ...
            if assets in MULTI_CAM_ASSETS:
                ...
                # Optionally restrict to specified cameras
                if args.cameras:
                    requested = set(args.cameras)
                    missing = requested - set(camera_list)
                    if missing:
                        logging.warning(
                            "Requested cameras %s not in capture %s (available: %s), skipping missing",
                            missing,
                            capture,
                            camera_list,
                        )
                    camera_list = [c for c in camera_list if c in requested]
